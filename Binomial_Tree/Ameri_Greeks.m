function [delta_call, gamma_call, vega_call, rho_call, theta_call, ...
          ...
          Ameri_Greeks(S, K, r, sigma, T, N, q)
%This is a function to calculate the Greeks of American call and put options.
...
% Please enter S, K, r, sigma, T, N, q in order.
   ...
   [~,~,V_call] = American_option_binomial(S, K, r, sigma, T, N, q);
   [~,~,~,V_put] = American_option_binomial(S, K, r, sigma, T, N, q);
   [~,~,~,~,St] = American_option_binomial(S, K, r, sigma, T, N, q);
   % Calculate call and put option prices when the sigma changes. 
   [callpx_plu_sig, putpx_plu_sig] = American_option_binomial(S, K, r, sigma+diff_sig, T, N, q);
   [callpx_min_sig, putpx_min_sig] = American_option_binomial(S, K, r, sigma-diff_sig, T, N, q);
   % Calculate call and put option prices when the r changes.
   [callpx_plu_r, putpx_plu_r] = American_option_binomial(S, K, r+diff_r, sigma, T, N, q);
   [callpx_min_r, putpx_min_r] = American_option_binomial(S, K, r-diff_r, sigma, T, N, q);
